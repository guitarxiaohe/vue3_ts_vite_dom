...
import { getChatPromptFormFields } from './constants';
...
const formFields = computed(() =>
  mapEntityFormFields(getChatPromptFormFields(t))
);
const formChildren = computed(
  () => getEntityTableConfig('chatPrompt').children ?? []
);
const resolvedRowKey = computed(() => {
  const config = getEntityTableConfig('chatPrompt');
  return config.rowKey || 'id';
});
const drawerTitle = computed(() => {
  if (props.isCreate && props.record) {
    return `${t('common.copy')}${t('entity.chatPrompt.title')}`;
  }

  return props.isCreate
    ? `${t('common.add')}${t('entity.chatPrompt.title')}`
    : `${t('common.edit')}${t('entity.chatPrompt.title')}`;
});
